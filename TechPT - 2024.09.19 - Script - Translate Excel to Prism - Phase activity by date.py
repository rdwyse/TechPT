#We are going to rewrite this script to prepare data for prism, where the X values are the timedatestamp column Y values are Case1, Case2,Case3, and Case4. The data within each cell will either be null or phase_part.
...
if csv_file:
    try:
        ...
        #Rewrite: Phase_part should be added as the last column of the loaded CSV data or data frame if appropriate. Without any calculations. Simply Row 1 had a study phase of BL and a sessiontype of AC an entry of  BL_AC is entered into the Phase_Part column. this should be done for all rows.  


       #Rewrite: Now we are going to translate this part of the script to use prepare timedatestamp colum as the X values as opposed to Trial Block Count. 
        ...
        #Rewrite: For the update Change column_count and order to row.
        idx = 0
        row_count = 0
        rows_order = []
        ...
        while idx < len(df):
            ...
            last_column_value1 = row[last_column]  #Rewrite: Use the last column this should be phase_Part

            # Increment column count
            row_count += 1
            # Create a new column name
            # changed this to timedatestamp
            column_name = f"{time}_{column_count}"
        ...
        # Reorder columns
        #Rewrite: Change the desired_columns_order to row_order
        desired_columns_order = ['Isolated_TimeDateStamp'] + ["Case1","Case2","Case3","Case4"]
